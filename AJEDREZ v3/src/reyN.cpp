...
reyN::reyN(float x, float y, unsigned char r, unsigned char a, unsigned char v) {
	rojo = r;
	azul = a;
	verde = v;
	posxini = x;
...
void reyN::setcolor(unsigned char r, unsigned char a, unsigned char v) {
	rojo = r;
	azul = a;
	verde = v;
}

void reyN::setpos(float x, float y) {
	posxini = x;
	posyini = y;
}
...
int reyN::mov_correcto(float posxfin, float posyfin)
{
...
void reyN::mueve(float posxinif, float posyinif, float posxfinf, float posyfinf)
{
	if (posxinif == posxini && posyinif == posyini)
		movimientos(posxfinf, posyfinf);
}

void reyN::movimientos(float posxfin, float posyfin)
{
